...
def splitByLayerOld(fcToSplit, splitFC, countField, onlyKeepLargest, outputFC):
    ...
    fldsInput1 = [f.name for f in arcpy.ListFields(outputFC) if f.name not in (desc.shapeFieldName,desc.oidFieldName,shapeLengthFieldName)] + \
                 ["OID@","shape@"]
    ...
    tempFCName = Common.random_string_generator()
    tempFC= os.path.join(tempWorkspace, tempFCName)
    geoToLayerMap = arcpy.SpatialJoin_analysis(target_features=outputFC,
                                       join_features=splitFC,
                                      out_feature_class=tempFC,
                                      join_operation="JOIN_ONE_TO_MANY",
                                      join_type="KEEP_COMMON",
